...
class DetailedPDFExporter:
    ...
    def _add_employee_detail_to_pdf(self, pdf, report: Dict, font_name: str):
        ...
        pdf.cell(0, 10, self._fix_rtl(f"گزارش تفصیلی - {emp['full_name']} ({emp['user_id']})"), ln=True, align='C')
        pdf.set_font(font_name, '', 10)
        pdf.cell(0, 6, self._fix_rtl(f"دپارتمان: {emp['department']} | ماه: {report['month_name']} {report['year']}"),
                 ln=True, align='C')
        pdf.ln(3)
        ...
        headers = ['تاریخ', 'روز', 'وضعیت روز', 'وضعیت فرد', 'ورود', 'خروج', 'کارکرد', 'اضافه']
        col_widths = [22, 18, 18, 22, 13, 13, 13, 13]
