...
ALTER TABLE travel_leave_details
    ADD COLUMN IF NOT EXISTS membership_code_snapshot VARCHAR(10),
    ADD COLUMN IF NOT EXISTS marital_status_snapshot VARCHAR(1),
    ADD COLUMN IF NOT EXISTS distance_method_snapshot VARCHAR(30);

-- Existing application contract types: 1..7.
...
WHERE NOT EXISTS (
    SELECT 1 FROM travel_leave_policies p WHERE p.contract_type_code = v.code
);
...
SELECT
    p.id, r.min_km, r.max_km, r.travel_days, r.description, r.is_active
FROM travel_leave_policies p
CROSS JOIN travel_leave_policy_rules r
WHERE r.policy_id IS NULL;

-- Remove legacy global rule rows after their values have been copied.
...
SELECT
    p.id, s.marital_status, q.annual_max_usage,
    q.description,
    'annual_max_usage_' || s.marital_status,
...
WHERE NOT EXISTS (
    SELECT 1
    FROM travel_leave_quota_settings x
    WHERE x.policy_id = p.id AND x.marital_status = s.marital_status
);

-- Remove legacy global quota rows after their values have been copied.
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_tlqs_policy_status
    ON travel_leave_quota_settings(policy_id, marital_status);
