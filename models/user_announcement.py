...
class UserAnnouncement(TimestampMixin, Base):
    ...
    user_id: Mapped[str] = mapped_column(String(50), ForeignKey("users.user_id", ondelete="CASCADE"), nullable=False, index=True)
    announcement_id: Mapped[int] = mapped_column(Integer, ForeignKey("system_announcements.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    announcement: Mapped["SystemAnnouncement"] = relationship("SystemAnnouncement", back_populates="user_read_states")

    def __repr__(self) -> str:
        return f"<UserAnnouncement(user_id='{self.user_id}', announcement_id={self.announcement_id}, seen_at={self.seen_at})>"

    def to_dict(self) -> dict:
        return {
            "id": self.id, "user_id": self.user_id, "announcement_id": self.announcement_id,
