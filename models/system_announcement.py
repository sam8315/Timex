...
class SystemAnnouncement(TimestampMixin, Base):
    ...
    version: Mapped[Optional[str]] = mapped_column(String(30), nullable=True, index=True)
    title: Mapped[str] = mapped_column(String(200), nullable=False)
    summary: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)
    content: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    announcement_type: Mapped[str] = mapped_column(String(30), nullable=False, default="general", index=True)
    ...
    created_by: Mapped[Optional[str]] = mapped_column(String(50), ForeignKey("users.user_id", ondelete="SET NULL"), nullable=True, index=True)

    user_read_states: Mapped[list["UserAnnouncement"]] = relationship(
        "UserAnnouncement", back_populates="announcement", cascade="all, delete-orphan", lazy="selectin"
    )
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "announcement_type": self.announcement_type, "is_active": self.is_active,
